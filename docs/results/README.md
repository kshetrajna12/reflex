...
| 09-22 | [large-choice](large-choice.md) | Show a choice of more than 26 options as pages of 26 in one pass, pooled into a single softmax, so the 14,500 Decision Index requests the 26-option cap refused can be answered. | **The cap is gone.** All 16,088 frozen rows of the five refused benchmarks answer, none unsupported: CLINC150+OOS 0.621 at 151 options, API-Bank 0.612, BANKING77 0.495, ChessBench 0.110, POP909-CL 0.048. Questions of 2 to 26 options are unchanged bit for bit (650 probabilities, max difference 0). Latency grows ~230 ms per page. Worth about +0.26 index, because the two benchmarks inside the panel are the two the 4B is worst at. |
