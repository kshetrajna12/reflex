...
| 09-22 | [decision-index](decision-index.md) | Wire reflex into the Decision Index harness (132,422 requests, 37 benchmarks) and run its 100-request sample against `stable`. | The format is clean: 83 of 83 supported rows answered and validated. Two capacity gaps found and fixed on `main`: refusals were worded so the harness read them as server errors (`6dce470`), and the 4096-token one-question ceiling, which refuses all of ToolRet and BRIGHT, was not settable from the command line (`e8af022`). **Blocked**: the frozen suite dataset is 404 and a rebuild would not be submittable. |
